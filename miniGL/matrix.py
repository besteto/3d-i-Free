...
class mat3:
    ...
    def __init__(self):
        self.__M = identity(3, dtype=float32)
    ...
    def translate(self, x, y, z):
        self.__M = self.__M * matrix([[1, 0, 0, x], [0, 1, 0, y], [0, 0, 1, z]], dtype=float32)

    def scale(self, x, y, z):
        self.__M = self.__M * matrix([[x, 0, 0, 0], [0, y, 0, 0], [0, 0, z, 0]], dtype=float32)

    def rotate_x(self, a):
        s = sin(radians(a))
        c = cos(radians(a))
        self.__M = self.__M * matrix([[1, 0, 0, 0], [0, c, -s, 0], [0, s, c, 0]], dtype=float32)

    def rotate_y(self, a):
        s = sin(radians(a))
        c = cos(radians(a))
        self.__M = self.__M * matrix([[c, 0, s, 0], [0, 1, 0, 0], [-s, 0, c, 0]], dtype=float32)

    def rotate_z(self, a):
        s = sin(radians(a))
        c = cos(radians(a))
        self.__M = self.__M * matrix([[c, -s, 0, 0], [s, c, 0, 0], [0, 0, 1, 0]], dtype=float32);
    ...
    def perspective(self, fov, aspect, n, f):
        ...
        dz = f - n
        self.__M = matrix(
            [[1. / w, 0, 0, 0], [0, 1. / h, 0, 0], [0, 0, -(f + n) / dz, -2. * f * n / dz]],
            dtype=float32)

    def transpose(self):
        t = mat3().copy(self.__M)
        t.raw.transpose()
        return t

    def inverse(self):
        return mat3().copy(linalg.inv(self.__M))